...
## No SNMP Data
Telegraf depends on SNMP data from the Edge Appliances to work. You can confirm that SNMP is configured and working on the Edge Appliance by connecting to he server where you installaed the TIG stack and using the snmpwalk utility to load SNMP data. Here's the command for SNMPv3:
     ```shell
